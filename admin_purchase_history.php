...
?>
    ...
    <div class="az-content az-content-dashboard-nine">
      ...
      <div class="az-content-header">
        <div class="az-content-header-top">
          <div>
            <h2 class="az-content-title mg-b-5 mg-b-lg-8">PURCHASE HISTORY OF STUDENT</h2>
          </div>
            ...
            <div class="card">
              ...
              <thead>
                <tr>
                  ...
                  <th class="wd-20p">PURCHASE DATE</th>
                  <th class="wd-20p">TOTAL PRICE</th>
                  <th class="wd-25p">PAYMENT TYPE</th>
  ...
  <?php
$i = 0;
/**SELECT * FROM mdl_enrol_paypal inner join mdl_user on mdl_user.id=mdl_enrol_paypal.userid left join mdl_user_enrolments on mdl_user_enrolments.userid=mdl_user.id left join mdl_enrol on mdl_enrol.id=mdl_user_enrolments.enrolid**/
                    $sql3="SELECT u.firstname,ep.txn_id,cc.id as cat_id, c.id as course_id, FROM_UNIXTIME(ep.timeupdated) as purchase_date, ep.payment_status,ep.item_name FROM `mdl_enrol_paypal` as ep inner join mdl_course as c ON ep.courseid = c.id inner join mdl_course_categories AS cc ON c.category = cc.id inner join mdl_user as u on u.id=ep.userid ";
...
$i++;
                     ...
                     // $id = $row1['userid'];
                      $payment_id = $row1['txn_id'];
                      $course = $row1['item_name'];
                         $payment_type = 'PAYPAL';
                      //$purchase_date = $row1[''];
                       //$purchase_date = $row1[''];
                       //$purchase_date = $row1['item_name'];
                       $status = $row1['payment_status'];
 $purchase_date = $row1['purchase_date'];
