...
while($parent = mysqli_fetch_array($mysqla)) {
  ...
  $sqlb="SELECT distinct c.fullname As courseid,cc.path,cc.name FROM mdl_course c left outer join mdl_user_lastaccess l on (c.id=l.courseid OR l.courseid IS NULL) join mdl_course_categories as cc on cc.id=c.category WHERE DATEDIFF(CURDATE(), FROM_UNIXTIME(l.timeaccess)) >= 120 and cc.path LIKE '/".$userRow["inst_id"]."/%' ";
