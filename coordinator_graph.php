    ...
    <div class="az-content az-content-dashboard-nine">
      ...
      <div class="az-content-header">
        <div class="az-content-header-top">
          ...
          <div>
           <h2 class="az-content-title mg-b-5 mg-b-lg-8">Coordinator Information</h2>
...
Highcharts.chart('container', {
  ...
  $sqlb="SELECT count(distinct u.firstname) as fname,month(from_unixtime(ra.timemodified) )  as month,year(from_unixtime(ra.timemodified )) as year FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=ra.userid JOIN mdl_course_categories AS cats ON c.category = cats.id join mdl_enrol as en on en.courseid=c.id where ra.roleid='2'  and DATEDIFF(NOW(),FROM_UNIXTIME(ra.timemodified) ) <= '$pay' and cats.path  LIKE '/".$userRow["inst_id"]."/%' group by month,year order by year,month ";
...
$stud="SELECT count(u.firstname) as fname ,,DATE_FORMAT(FROM_UNIXTIME(ra.timemodified), '%M') as month FROM `mdl_course_categories` as cc inner join mdl_context as cnt on cnt.instanceid=cc.id inner join mdl_role_assignments as ra on ra.contextid=cnt.id inner join mdl_role as r on r.id=ra.roleid inner join mdl_user as u on u.id=ra.userid where r.id=2 and cc.path LIKE '/".$userRow["inst_id"]."'  and  group by month ";
