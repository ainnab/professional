...
$sqlv="SELECT distinct c.fullname,cats.name FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=ra.userid  JOIN mdl_course_categories AS cats ON c.category = cats.id where u.username='".$var["username"]."'";
$mysqlv = mysqli_query($dbconfig,$sqlv);
$rowv =mysqli_fetch_array($mysqlv,MYSQLI_ASSOC);

  $course = $rowv['fullname'];
