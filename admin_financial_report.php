...
$query="SELECT u.username as username,u.firstname,u.lastname, u.email, cc.name as institution ,r.name as rolename, cc.id as inst_id, ra.roleid,ra.userid
  ...
  $res1=mysqli_query($dbconfig,$query);
  //echo $sql_query;
  $userRow1=mysqli_fetch_array($res1,MYSQLI_ASSOC);
  $username = $userRow1['username'];
  $userId = $userRow1['userid'];
  $userRole = $userRow1['roleid'];
  $name = $userRow1['firstname'];
  $lastname = $userRow1['lastname'];
  $role = $userRow1['rolename'];
  $email = $userRow1['email'];
  $institution = $userRow1['institution'];
  $inst_id = $userRow1['inst_id'];
 
  
  $earn = 0;
  $sql_query1 = "SELECT distinct e.cost as cost,ep.txn_id,cc.parent FROM `mdl_enrol_paypal` as ep inner join mdl_enrol as e ON e.courseid = ep.courseid inner join mdl_course as c ON ep.courseid = c.id inner join mdl_course_categories AS cc ON c.category = cc.id where e.cost !='NULL';
          ";
  //echo $sql_query1;
      ...
      <div class="az-content-body">
        ...
        <hr>
              ...
              <div class="card-body row pd-25">
                ...
                <p class="tx-gray-600 mg-b-0">Details of payment record from the ILMU UMP according to the agreed profit percentage </p>
              </div><!-- card-header -->
              <div class="card-body">
        ...
        <?php
        $sql_query2="SELECT ep.txn_id,cc.id as cat_id, c.id as course_id, FROM_UNIXTIME(ep.timeupdated) as purchase_date, ep.payment_status,ep.item_name FROM `mdl_enrol_paypal` as ep inner join mdl_course as c ON ep.courseid = c.id inner join mdl_course_categories AS cc ON c.category = cc.id";
        //echo $sql_query2;
      ...
      ?>
              <tbody>
                ...
                <tr>
                  ...
                  <td><span class="badge badge-pill badge-success"><?php echo $payment_status?></span></td>
          <td><a href="receipt.php"><button class="btn btn-outline-indigo btn-block">Receipt</button></a></td>
                </tr>
