...
=======
         <?php $result = mysqli_query($dbconfig,"SELECT * FROM student left join programselection as pg on pg.stud_ic=student.stud_ic where student.role='student' and pg.status='Applied' order by pg.stud_ic"); ?>
>>>>>>> Stashed changes
...
=======
                            ...
                            <?php if ($row['status'] == 'Approved' || $row['status'] == 'Submitted' || $row['status'] == 'Rejected') { ?>
>>>>>>> Stashed changes
   ...
   ?>
    ...
    <?php while($data = mysqli_fetch_array($records)) { ?>
         <?php $result = mysqli_query($dbconfig,"SELECT * FROM student where role='student' AND (modeAds='ROPE' AND app_status='Applied');"); ?>
            <?php if (mysqli_num_rows($result) > 0) { ?>
