    ...
    <div class="az-content az-content-dashboard-nine">
      ...
      <div class="az-content-header">
        <div class="az-content-header-top">
          <div>
            <h2 class="az-content-title mg-b-5 mg-b-lg-8">STUDENT INFORMATION</h2>
          </div>
            ...
            <div class="card">
              ...
              <thead>
                <tr>
                  ...
                  <th class="wd-25p">STUDENT NAME</th>
                  <th class="wd-20p">COURSE NAME</th>
                   <th class="wd-20p">INSTRUCTOR</th>
                  <th class="wd-15p">FREE/PAID</th>
                  <th class="wd-20p">ENROLLED DATE</th>
...
$sqlb="SELECT distinct cats.path,c.id,c.fullname,us.username,tra.roleid FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS tra ON tra.contextid = ctx.id join mdl_user as us on us.id=tra.userid JOIN mdl_course_categories AS cats ON c.category = cats.id WHERE tra.roleid=5 and cats.path LIKE '/".$parent["id"]."/%' ";
...
$sqlt="SELECT distinct cats.path,c.id,c.fullname,us.username as tec ,tra.roleid FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid  JOIN mdl_role_assignments AS tra ON tra.contextid = ctx.id join mdl_user as us on us.id=tra.userid JOIN mdl_course_categories AS cats ON c.category = cats.id
WHERE  tra.roleid=9 and c.id='".$var["id"]."'";

 $resultt=mysqli_query($dbconfig,$sqlt);
...
$sql2="SELECT distinct  FROM_UNIXTIME(ue.timestart) as enrol_date,en.cost FROM `mdl_user_enrolments` as ue join mdl_enrol as en on en.id=ue.enrolid join mdl_course as co on co.id=en.courseid join mdl_user as u on u.id=ue.userid where u.username='".$var["username"]."'
                       ...
                       $sql2="SELECT FROM_UNIXTIME(ue.timestart) as enrol_date,en.cost FROM `mdl_user_enrolments` as ue join mdl_enrol as en on en.id=ue.enrolid join mdl_course as co on co.id=en.courseid join mdl_user as u on u.id=ue.userid where u.username='".$var["username"]."'
...
echo "<tr>
<td>$var[username]</td>
<td>$var[fullname]/$name[name]</td>
<td>$rowt[tec]</td>
<td>";
if($cost =='') {
echo "<label class='badge badge-info'>FREE</label>"; } 
if($cost =='NOT NULL') {
echo "<label class='badge badge-info'>PAID</label>"; } 


echo "</td>";
                     
echo "<td>".$row2["enrol_date"]."</td>";
