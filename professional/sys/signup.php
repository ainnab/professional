...
.az-column-signup {
 ...
 /*   $phone = $_POST['phone'];*/
    ...
    $sql_u = "SELECT * FROM student WHERE stud_ic='$ic'";
  	$res_u = mysqli_query($dbconfig2, $sql_u);
    $dte = date('d/m/Y');
    ...
    if (mysqli_num_rows($res_u) > 0) {
		...
		$sql = "insert into student (stud_name, stud_ic, password, stud_email, role, app_status, intake, date, app_option) value('".$name."', '".$ic."','".$hashPassword."','".$email."','student','Applied','sem1 22/23','".$dte."','0')";
		$result = mysqli_query($dbconfig2, $sql);
		if($result)
		{
      $sql2 = "insert into parent (stud_ic) value('".$ic."')";
		  $result2 = mysqli_query($dbconfig2, $sql2);
      if($result2){
        $sql3 = "insert into qualification_info (stud_ic) value('".$ic."')";
		    $result3 = mysqli_query($dbconfig2, $sql3);}
        if($result3){
         $sql4 = "insert into application (stud_ic, intake) value('".$ic."','sem1 22/23')";
		     $result4 = mysqli_query($dbconfig2, $sql4);}
          if($result4){
            $sql5 = "insert into data_ptmk (NOKP,NOKPFIN) value('".$ic."','".$ic."')";
		        $result5 = mysqli_query($dbconfig2, $sql5);
          
        echo "<script type='text/javascript'>alert('Registration Successfully!'); window.location.href = 'signin.php';</script>";
      }
      else{
        echo "opps! something was wrong.". mysqli_error($dbconfig2);;
      }
