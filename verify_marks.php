...
?>
 ...
 <table id="example2" class="table">
        ...
        $sql_query2="SELECT distinct c.id,c.fullname,u.id as studentid,u.username as student,us.username ,tra.roleid,usr.username as teacher ,ra.roleid,usr.institution as inst FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS lra ON lra.contextid = ctx.id JOIN mdl_role_assignments AS tra ON tra.contextid = ctx.id JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=lra.userid join mdl_user as us on us.id=tra.userid join mdl_user as usr on usr.id=ra.userid WHERE lra.roleid=5 and ra.roleid=9 and tra.userid ='$userId' and c.id=258 ";
         //echo $sql_query2;
        $res2=mysqli_query($dbconfig,$sql_query2);
        while($userRow2=mysqli_fetch_array($res2,MYSQLI_ASSOC)) {
          //echo "TEST";
        $courseName = $userRow2['fullname'];
         ...
         $cid = $userRow2['id'];
        $inst = $userRow2['inst'];
        $studID = $userRow2['studentid'];
 
                   
                   $sql2="SELECT FROM_UNIXTIME(ue.timestart) as enrol_date,en.cost FROM `mdl_user_enrolments` as ue join mdl_enrol as en on en.id=ue.enrolid join mdl_course as co on co.id=en.courseid join mdl_user as u on u.id=ue.userid where u.username='$student'
                ...
                <tr> 
                 ...
                 <td><div class="dropdown">
                      ...
                      <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
                        ...
                        <a class="dropdown-item" href="verify_studentsMark.php?id=<?php echo $studID;?>"><i class="dw dw-eye"></i> Verify Marks</a>
                      </div>
                    </div></td>
