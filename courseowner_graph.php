...
Highcharts.chart('container', {
  ...
  $sqlb="SELECT distinct u.firstname as fname,month(from_unixtime(ra.timemodified) ) as month,year(from_unixtime(ra.timemodified )) as year FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=ra.userid JOIN mdl_course_categories AS cats ON c.category = cats.id join mdl_enrol as en on en.courseid=c.id where ra.roleid='3' and ctx.contextlevel='50' and cats.path  LIKE '/".$userRow["inst_id"]."/%' group by month,year order by year,month  ";
...
$stud="SELECT count(distinct u.firstname) as fname,month(from_unixtime(ra.timemodified) ) as month,year(from_unixtime(ra.timemodified )) as year  FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=ra.userid JOIN mdl_course_categories AS cats ON c.category = cats.id join mdl_enrol as en on en.courseid=c.id where ra.roleid='3' and ctx.contextlevel='50' and DATEDIFF( NOW(),FROM_UNIXTIME(ra.timemodified) ) <= '$pay' and cats.path  LIKE '/".$userRow["inst_id"]."/%' group by month,year order by year,month ";
