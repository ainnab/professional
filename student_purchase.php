...
/**SELECT * FROM mdl_enrol_paypal inner join mdl_user on mdl_user.id=mdl_enrol_paypal.userid left join mdl_user_enrolments on mdl_user_enrolments.userid=mdl_user.id left join mdl_enrol on mdl_enrol.id=mdl_user_enrolments.enrolid**/
                    $sql3="SELECT u.firstname,ep.txn_id,cc.id as cat_id, c.id as course_id, FROM_UNIXTIME(ep.timeupdated) as purchase_date, ep.payment_status,ep.item_name FROM `mdl_enrol_paypal` as ep inner join mdl_course as c ON ep.courseid = c.id inner join mdl_course_categories AS cc ON c.category = cc.id inner join mdl_user as u on u.id=ep.userid where u.username='$login_session'";
