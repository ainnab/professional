...
<?php
        $stud="SELECT ct.name FROM mdl_course_categories ct JOIN mdl_course_categories basecat ON basecat.id = ct.id JOIN mdl_course c ON c.category = ct.id join mdl_context as cnt on cnt.instanceid=ct.id WHERE ct.visible = '1' AND ct.path LIKE '/".$userRow["inst_id"]."/%' group by ct.name";



$sqlst = mysqli_query($dbconfig,$stud);
while($co = mysqli_fetch_array($sqlst)){
//$path = explode("/", $co["path"]);
//$courseName=$co['fullname'];
$category=$co['name'];

$stud1="SELECT ct.path FROM mdl_course_categories ct JOIN mdl_course_categories basecat ON basecat.id = ct.id JOIN mdl_course c ON c.category = ct.id join mdl_context as cnt on cnt.instanceid=ct.id WHERE   ct.name ='".$co["name"]."'  ";


$sqlst1 = mysqli_query($dbconfig,$stud1);
$co1 = mysqli_fetch_array($sqlst1,MYSQLI_ASSOC);

$path=$co1['path'];
$path = explode("/", $co1["path"]);
...
?>
    ...
    series: [{
        ...
        data: [
     <?php
        $stud="SELECT count(distinct c.fullname) as fullname,ct.name FROM mdl_course_categories ct JOIN mdl_course_categories basecat ON basecat.id = ct.id JOIN mdl_course c ON c.category = ct.id join mdl_context as cnt on cnt.instanceid=ct.id WHERE ct.visible = '1' AND ct.path LIKE '/".$userRow["inst_id"]."/%' group by ct.name";
...
?>
<?php echo "[";?> '<?php echo $category; ?>',
  <?php echo $courseName; ?>,<?php echo "],";?> 
...
?>
       ...
       /** $query = mysqli_query($dbconfig,"SELECT count(distinct us.username) as st,DATE_FORMAT(FROM_UNIXTIME(ue.timestart), '%M') as MONTH FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS tra ON tra.contextid = ctx.id join mdl_user as us on us.id=tra.userid Join mdl_user_enrolments as ue on ue.userid=us.id JOIN mdl_course_categories AS cats ON c.category = cats.id WHERE tra.roleid=5  GROUP BY MONTH(FROM_UNIXTIME(ue.timestart)) order by MONTH(FROM_UNIXTIME(ue.timestart))");
