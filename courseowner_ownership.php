    ...
    <div class="az-content az-content-dashboard-nine">
      ...
      <div class="az-content-header">
        <div class="az-content-header-top">
          <div>
            <h2 class="az-content-title mg-b-5 mg-b-lg-8">Manager Ownership</h2>
      ...
      <div class="col-xl-12 mg-t-15 mg-t-20">
            <div class="card">
              <div class="card-header">
                <h6 class="card-title tx-14 mg-b-5"></h6>
              
              </div><!-- card-header -->
              <div class="card-body">
                <div class="table-responsive">
          <table id="example1" class="table table-striped mg-b-0">
            <thead>
            <tr>
          ...
          $sql_query2="SELECT ep.txn_id,cc.id as cat_id, c.id as course_id, FROM_UNIXTIME(ep.timeupdated) as purchase_date, ep.payment_status,ep.item_name,cc.name as course_owner,e.cost as cost, ep.userid,u.firstname,u.lastname FROM `mdl_enrol_paypal` as ep
                ...
                WHERE ep.courseid = '$course_id' AND e.cost is not null";
          //echo $sql_query2;
          ...
          //$cost = $userRow2['cost'];
          $userid = $userRow2['userid'];
          $payment_type = 'PAYPAL';


           $sql_query3 = "SELECT COUNT(distinct e.cost) as enrol FROM `mdl_enrol_paypal` as ep
                inner join mdl_enrol as e ON e.courseid = ep.courseid
                inner join mdl_course as c ON ep.courseid = c.id 
                inner join mdl_course_categories AS cc ON c.category = cc.id
                WHERE c.id = '$course_id' AND e.cost is not null";
        //echo $sql_query1;
        $res3=mysqli_query($dbconfig,$sql_query3);
        //$enrol=mysqli_num_rows($res1);
        $userRow3=mysqli_fetch_array($res3,MYSQLI_ASSOC);
       
        $enrol = $userRow3['enrol'];
        
        $sqlb="SELECT distinct u.firstname as fname ,u.username ,u.lastname as lname ,cats.path,c.fullname,cats.name ,ra.roleid,u.department FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=ra.userid JOIN mdl_course_categories AS cats ON c.category = cats.id where ra.roleid='3' and ctx.contextlevel='50' and c.id='$course_id' ";


$mysqlb = mysqli_query($dbconfig,$sqlb);
$var = mysqli_fetch_array($mysqlb,MYSQLI_ASSOC);
...
$uname =$var['username'];
  ...
  $sqlc"SELECT course_owner,course_name,code,co_weight FROM cs_owner where course_owner='$uname' and code='$course_id' ";


$mysqlc = mysqli_query($dbconfig2,$sqlc);
$var1 = mysqli_fetch_array($mysqlc,MYSQLI_ASSOC);

$weight = $var1['co_weight'];


$total=$enrol*10/100;   
 $total_course=$total*50/100;
          
          ?>
          
            <th class="wd-15p">Course Name</th>
            <th class="wd-20p">Coordinator</th>
            <th class="wd-20p">Total Ownership</th>
            ...
            <td>RM <?php   echo $total_weight=$weight*$total_course/100  ?></td>
