...
$test="SELECT count(distinct u.username) as username ,cats.path,c.fullname,cats.name ,ra.roleid,DATE_FORMAT(FROM_UNIXTIME(ue.timestart), '%M') as MONTH FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=ra.userid join mdl_user_enrolments as ue on ue.userid=u.id JOIN mdl_course_categories AS cats ON c.category = cats.id where ra.roleid='5' and c.fullname='".$var["fullname"]."' ";



$sqltest = mysqli_query($dbconfig,$test);
$co1 = mysqli_fetch_array($sqltest,MYSQLI_ASSOC);

$student=$co1['username'];
}
}
...
$test="SELECT count(distinct u.username) as username FROM mdl_course AS c LEFT JOIN mdl_context AS ctx ON c.id = ctx.instanceid JOIN mdl_role_assignments AS ra ON ra.contextid = ctx.id join mdl_user as u on u.id=ra.userid join mdl_user_enrolments as ue on ue.userid=u.id JOIN mdl_course_categories AS cats ON c.category = cats.id where ra.roleid='5'  and cats.name='".$name["name"]."' ";
